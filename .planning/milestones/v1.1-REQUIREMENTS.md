# Requirements Archive: v1.1 Scale Generation & Library
...
- [x] **SURF-01**: User can open a dedicated Generate page (`/pages/generate`) from the site nav
- [x] **SURF-02**: User can choose a generation method from a family-grouped picker; the parameter panel swaps to that method's inputs
- [x] **SURF-03**: User sees a live preview table and can audition (play) the current generated scale as parameters change
...
- [x] **SYNC-01**: User can send a generated scale to the Dashboard ("Send to Dashboard") and it loads there live
- [x] **SYNC-02**: User can send a generated scale to the Analysis page ("Send to Analysis") and it loads there live
- [x] **SYNC-03**: A sent scale persists across reload via the shared store and the existing `#s=` deep-link hash
- [x] **SYNC-04**: Existing Dashboard / Analysis behavior is byte-identical when no scale has been sent (empty-store boot regression guard)
...
*Last updated: 2026-06-14 — v1.1 milestone complete. All 23 requirements delivered and verified (Phases 5–9, 100% coverage). Phase-5 SURF/SYNC rows reconciled from Pending → Complete at close.*
</content>
