...
status: shipped
stopped_at: ""
last_updated: "2026-05-12T20:55:00.000Z"
last_activity: 2026-05-12 -- quick-260512-ilb: add Plot.barX comma-magnitude bar chart (Pythagorean ≈23.46¢ / syntonic ≈21.51¢ / schisma ≈1.95¢ with schisma bar highlighted red against blue parent commas, x-domain [0,26]¢, labels to 2 d.p. read from `pythagorean.cents`/`syntonic.cents`/`schisma.cents` — no hardcoded literals, BigInt-Fraction stays source of truth) + Schismatic-temperament playDyad section (replaces prior single tempering paragraph; new Interval bindings `schismaticThird = "8192/6561"` ≈ 384.36¢ and `fiveLimitThird = "5/4"` ≈ 386.31¢, exactly one schisma apart; prose covers eight-fifths-down-five-octaves-up identity + equivalent `(4/3)^8/2^3` framing + Helmholtz 0.24¢-per-fifth narrowing recipe yielding 701.71¢ fifth; inline `playDyad(schismaticThird, fiveLimitThird, synth, {label:"8192/6561 + 5/4 (schisma beat)"})` button hears the ~2 Hz schisma-rate beat against the major-third chord) + `## Further reading` section appended after `## See also` (Xenharmonic Wiki Schisma + IMSLP Helmholtz *Die Lehre von den Tonempfindungen* using canonical `Helmholtz,_Hermann_von` URL with `_von` suffix); math correction during planning — user's original `(3/2)^8 / 2^4 = 6561/4096` formula is one schisma sharp of 8/5 not 5/4, replaced with `8192/6561` after three-source verification (Tonalsoft, Wikipedia *Schismatic temperament*, Xenharmonic Wiki *Schismatic family*); single file edit src/pages/schisma.md +107/−4; tsc clean; vitest 293/293 across 23 files; prettier clean; `npm run build` clean (79 links validated); page bundle 11kB→23kB and 391kB→916kB matching Plot-using pages footprint (comma-pump 912kB, pythagorean-comma 920kB, edo-approximation 913kB)
progress:
  total_phases: 4
  completed_phases: 4
  total_plans: 25
...
Last activity: 2026-05-12 -- quick-260512-ilb: add Plot.barX comma-magnitude bar chart (Pythagorean ≈23.46¢ / syntonic ≈21.51¢ / schisma ≈1.95¢ with schisma bar highlighted red against blue parents, x-domain [0,26]¢, labels read from `pythagorean.cents`/`syntonic.cents`/`schisma.cents` — no hardcoded literals) + `## Schismatic temperament` playDyad section (replaces prior single tempering paragraph; new bindings `schismaticThird = "8192/6561"` ≈ 384.36¢ and `fiveLimitThird = "5/4"` ≈ 386.31¢ exactly one schisma apart; prose covers eight-fifths-down-five-octaves-up identity + Helmholtz 0.24¢-per-fifth narrowing recipe; inline playDyad button hears ~2 Hz schisma beat against major-third chord) + `## Further reading` section (en.xen.wiki/w/Schisma + IMSLP Helmholtz *Die Lehre von den Tonempfindungen* canonical `Helmholtz,_Hermann_von` URL); math correction during planning — user's original `(3/2)^8 / 2^4 = 6561/4096` formula is one schisma sharp of 8/5 not 5/4, replaced with `8192/6561` after Tonalsoft + Wikipedia + Xenharmonic Wiki cross-check; single file edit +107/−4; tsc clean; vitest 293/293 across 23 files; prettier clean; observable build clean (79 links); page bundle 11kB→23kB and 391kB→916kB matching Plot-using pages footprint
