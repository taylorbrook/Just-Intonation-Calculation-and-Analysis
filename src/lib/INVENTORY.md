...
| `toMonzo`, `monzoToBigNumeratorDenominator`, `primeLimit`, `monzosEqual`, `tenneyHeight`, `PRIMES` | `xen-dev-utils@0.13.1` | Re-exported via `src/lib/monzo.ts`. R-01 reminder: re-exports list does NOT include `Fraction`. `monzosEqual` is length-tolerant (Pitfall #14). (260619-f7n / D1: the unused `wilsonHeight`, `monzoAdd`, `monzoSub`, `monzoScale` re-exports were dropped — zero repo-wide consumers.) |
| `benedettiHeight` | Custom (this repo) | xen-dev-utils ships `tenneyHeight` (log(n*d)) but not Benedetti (= n*d itself). Returns the EXACT `bigint` product `n*d` via `monzoToBigNumeratorDenominator` — no precision ceiling (Mercator's comma and larger stay exact past Number.MAX_SAFE_INTEGER). |
...
### Phase 9 — Scala archive index + data loader (Plan 09-01 / LIB-01)

The first Framework `*.json.ts` data loader in the repo. ALL build-time-pure logic lives
in the unit-tested `src/lib/scala-archive.ts` (covered by the existing `src/lib/**` vitest
glob); the loader `src/data/scala-archive.json.ts` is a thin `node:fs` shell with no logic
to test (D-A5). WRAP-don't-reimplement discipline (Pitfall #5): the index parses every
vendored `.scl` through the already-hardened `parseScl` (`scala.ts`), never a second parser.

| Symbol | Source | Notes |
|--------|--------|-------|
| `ArchiveEntry` (interface, `src/lib/scala-archive.ts`) | Custom (this repo) | LIB-01 / D-A3 entry shape `{ filename, name, pitchCount, tempered, degrees }`. `name` = the parseScl description (search field, empty allowed); `pitchCount` = `intervals.length - 1` (excludes the implicit auto-prepended 1/1); `degrees` = the serialized pitch lines AFTER parseScl, leading 1/1 dropped, ready to feed `parseScala` on the client. `tempered` is the LOAD-BEARING per-entry flag (D-A4 / SURF-06). |
| `isTemperedScl` (`src/lib/scala-archive.ts`) | Custom (this repo) | D-A4 / SURF-06. Build-time tempered test: true iff ANY non-comment, non-header pitch line contains a `.` (the cents marker, identical to `parsePitchToken`'s D-19 rule so the build flag can NEVER disagree with the runtime parser). Mirrors scala.ts line discipline (`!` comments + the description/count header excluded). F01 (all ratios) → false; F02 (all cents) + F03 (mixed) → true. |
| `buildArchiveIndex` (`src/lib/scala-archive.ts`) | Custom (this repo) — WRAPS `parseScl` (scala.ts) | LIB-01. `{ filename, text }[] → ArchiveEntry[]`. Delegates ALL .scl parsing to `parseScl` (never reimplements — Pitfall #5). A file that THROWS in parseScl (malformed/oversized/count-mismatch) is SKIPPED + `console.warn`ed so one bad vendored file never fails the whole build (T-09-01 mitigation). Degree serialization follows the generate.md `ratioPerLine`/`centsPerLine` convention: tempered entries emit `iv.cents.toFixed(4)` per line (cents source of truth), exact entries emit `${iv.fraction.n}/${iv.fraction.d}` (the writeScl `formatRatio` precedent — 2/1 not "2"). |
| `searchArchive` (`src/lib/scala-archive.ts`) | Custom (this repo) | LIB-01. Case-insensitive substring filter over `name` + `filename`, capped at `cap`; blank term returns the first `cap` entries; result length never exceeds `cap`; result order is stable (input order). The debounced-search consumer in 09-02 calls this per keystroke. |
| `DEFAULT_SEARCH_CAP` (`src/lib/scala-archive.ts`) | Custom (this repo) | LIB-01. `50` — the default browse/search result cap the 09-02 widget imports. |
| `scala-archive.json` loader (`src/data/scala-archive.json.ts`) | Custom (this repo) — FIRST `*.json.ts` in the repo | LIB-01 / D-A2 / D-A5. Framework data loader: resolves `src/data/scala-archive/` via `import.meta.url`, `readdirSync` the `.scl` files (sorted for deterministic build order), `readFileSync` each UTF-8, calls `buildArchiveIndex`, `process.stdout.write(JSON.stringify(entries))`. NO network access — local `node:fs` only (the snapshot is committed so the build is offline/self-hostable). Framework runs `.ts` loaders via `tsx/esm` (config.js:37); the emitted static `scala-archive.json` is read on a page via `FileAttachment("scala-archive.json").json()`. |

### Phase 9 — Archive-browser widget (Plan 09-02 / LIB-01 + LIB-02)

| Symbol | Source | Notes |
|--------|--------|-------|
| `generateArchive`, `GenerateArchiveOpts`, `GenerateArchiveElement` (`src/components/generate-archive.ts`) | Custom (this repo) — consumes `searchArchive`/`DEFAULT_SEARCH_CAP` (09-01) + `parseScala` + `scaleTable`/`playScale` | LIB-01 + LIB-02. Pattern-2 factory `(synth, opts?) => HTMLElement` with `getScale()` / `isTempered()` (the same widget contract as `generateCs`), so the 09-03 Send-to wiring is pure plumbing. **D-B1 lazy load:** entries arrive via an `opts.loadEntries` async thunk kicked off in the constructor (a "Loading archive…" status shows until it resolves); the page passes `() => FileAttachment("scala-archive.json").json()` and tests pass an in-memory array — the component is unit-testable WITHOUT FileAttachment (default thunk resolves to `[]`). **D-B2 capped debounced search:** the search input is debounced ~150 ms before re-filtering via `searchArchive`; never renders more than `DEFAULT_SEARCH_CAP` rows; a "showing X of Y" caption reports rendered-vs-total (Y = full match count computed locally BEFORE the cap, so truncation is visible). **D-B3 tempered-aware preview:** on row-select, `new Scale(parseScala(entry.degrees.join("\n")))` → `scaleTable(scale, baseHz, { tempered: entry.tempered })` + `playScale(scale, synth, { baseHz })`; the per-entry build-time `tempered` flag (09-01 D-A4) drives both the cents-only display AND `isTempered()` (SURF-06 — a tempered scale is never laundered as ratios). A defensive parse error on select is caught → status region + prior preview preserved (T-09-05 / D-16). **D-B4 / T-09-03:** every dynamic value (archive `name`/`filename`, "X notes" meta, tempered/JI marker) renders via `createElement` + `textContent` — the grep gate keeps the unsafe-HTML count at 0. NO module-level state (Pattern 2); NO AudioContext at construction (the synth is passed in; ⏵⏵ Play comes from `playScale`). |

## Quick-task entries

### quick-260824-bfz — note-name → Hz projection (`src/lib/pitch.ts`)

Pitfall #1 boundary, stated once for the whole section: EVERY symbol below produces a
**display / audio float**. These values are the reference anchor the exact rational scale
hangs from — never kernel input. A ratio round-tripped through Hz loses its prime
structure, so nothing here may be fed back into `Interval` / `Fraction` arithmetic.

| Symbol | Source | Notes |
|--------|--------|-------|
| `NOTE_NAMES`, `NoteName` (`src/lib/pitch.ts`) | Custom (this repo) | The twelve sharp-spelled chromatic names frozen in C-first order (C, C#, D, D#, E, F, F#, G, G#, A, A#, B); `NoteName` is the union derived from the tuple via `(typeof NOTE_NAMES)[number]`, so an unknown name fails at `tsc` rather than at runtime. C-first is the order scientific pitch notation requires — the octave number increments at C. |
| `A4_MIDI`, `DEFAULT_A4_HZ` (`src/lib/pitch.ts`) | Custom (this repo) | `69` and `440` — the anchor pair `midiToHz` calibrates against. `DEFAULT_A4_HZ` is exported (not inlined) because the Generate page's A4-calibration input needs it as the fallback when the user clears the field. |
| `noteToMidi` (`src/lib/pitch.ts`) | Custom (this repo) | `(note, octave) => (octave + 1) * 12 + NOTE_NAMES.indexOf(note)` — SCIENTIFIC PITCH NOTATION: **C4 is MIDI 60 and A4 is MIDI 69** (so C-1 is 0, the bottom of the MIDI range, and B4 is 71). Guards FIRST, in the `kbm.ts` style: a name absent from `NOTE_NAMES` and an octave that is not a finite integer both throw `RangeError` naming the offending value, so a bad input can never become a silent `NaN` downstream. |
| `midiToHz` (`src/lib/pitch.ts`) | Custom (this repo) — WRAPS `centOffsetToFrequency` (xen-dev-utils) | **Pitfall #5 outcome, recorded explicitly:** this WRAPS `centOffsetToFrequency` from `xen-dev-utils` — called as `centOffsetToFrequency((midi - A4_MIDI) * 100, a4Hz)` — rather than hand-rolling the exponential. Upstream is `centsToValue(offset) * baseFrequency` where `centsToValue` is `Math.pow(2, cents / 1200)`, so whole-octave offsets stay bit-exact: A3 reads exactly 220 and A5 exactly 880, no float dust. `mtof` from the SAME package was REJECTED: it hard-codes the A4 anchor at 440 with no calibration parameter, and a user-facing A4 knob (432 / 415 / 442) is the point of this module. Guards `midi` as finite and `a4Hz` as finite and strictly > 0 → `RangeError`. |
| `noteToHz` (`src/lib/pitch.ts`) | Custom (this repo) | The composition `midiToHz(noteToMidi(note, octave), a4Hz)` — the projection the Generate page's tonic picker rides on. `noteToHz("A", 4)` is EXACTLY 440 at the default calibration (the cent offset is zero), which is the anti-regression anchor: the page's shipped defaults (A / 4 / 440) leave `baseHz === 440` and first paint unchanged. |
| `formatNoteName` (`src/lib/pitch.ts`) | Custom (this repo) | Concatenates the name and the octave — "A4", "C#-1". Readout/tooltip display only; no parsing counterpart (the UI holds the note and octave as separate bound values, so nothing ever needs to re-parse this string). |
