...
| `benedettiHeight` | Custom (this repo) | xen-dev-utils ships `tenneyHeight` (log(n*d)) + `wilsonHeight` but not Benedetti (= n*d itself). Returns the EXACT `bigint` product `n*d` via `monzoToBigNumeratorDenominator` — no precision ceiling (Mercator's comma and larger stay exact past Number.MAX_SAFE_INTEGER). |
